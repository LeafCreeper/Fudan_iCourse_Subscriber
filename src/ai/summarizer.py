...
9. **PPT 图片插入**：
   - 在【PPT 文字识别】的每个页条目末尾，可能会出现 `![PPT 页 N](pptimg://N)` 形式的图片占位标记。这表示该页 PPT 有一张对应的原始幻灯片截图可用。
   - 你应该在总结中，当你认为某张 PPT 图片对理解内容有重要帮助时（例如包含复杂公式推导、图表、架构图、代码截图等难以仅用文字描述的内容），将该图片占位标记**原样复制**到输出 Markdown 的合适位置。
   - 插入原则：
     - 只插入对理解内容有实质性帮助的图片，不要把所有图片都插入。公式推导步骤、数据图表、流程图、结构图等适合插入；纯文字列表的 PPT 不需要插入。
     - 图片应紧跟在与其内容相关的段落或标题之后，不要集中堆放在文末。
     - 保持 `![PPT 页 N](pptimg://N)` 的格式完全不变，包括 `pptimg://` 前缀和页码数字。
     - 如果没有任何图片对理解有帮助，可以不插入任何图片占位标记。
   - 示例：在讲解某张含复杂电路图的 PPT 后，输出可能如下：
     ```
     ### 组合逻辑电路分析

     该电路由与门和或门组合而成，其逻辑表达式为 $F = AB + C$。下图展示了电路的完整连接关系：
