...
4. **公式与显示规范**：所有数学公式或科学变量必须使用规范的 LaTeX 语法（行内公式用 $...$，行间公式用 $$...$$）。数学分隔符必须严格成对，禁止输出半截 `$`、`$$`、`\(`、`\[`。无法确信正确的 OCR 公式请用普通文本或行内代码保留原样，不要强行改写成 LaTeX。由于图床限制，latex公式中不要出现中文。禁止输出 HTML、CSS、颜色、字号、居中等演示稿样式。
